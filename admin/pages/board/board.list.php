...
try {
    ...
    $query = "
        SELECT 
            a.no, a.sort_no, a.board_no, a.is_visible, a.is_contract, a.user_no, a.category_no, a.comment_cnt, a.title, a.contents, 
            ...
            a.extra16, a.extra17, a.extra18, a.extra19, a.extra20, a.extra21,  
                            ...
                            <div class="no-gallery">
                                <ul id="sort-gallery">
                                    <?php foreach ($arrResultSet as $v): 
                                        ...
                                        $thumb_image = "/uploads/board/". $v['thumb_image'];
                                        $address = $v['extra3'];
                                        $commission_fee = $v['extra20'];
                                        $pay_for_month = $v['extra5'];
                                        
                                    ?>

                                    <li class="no-gallery-item" data-no=<?=$v['no']?> data-sort-no=<?=$v['sort_no']?>>
                                        <a href="./board.view.php?no=<?= $v['no'] ?>&<?= $searchParam ?>">
                                            <div class="no-gallery-item-img">
                                                <figure>
                                                    <img src="<?=$thumb_image?>" alt="">
                                                </figure>
                                                <div class="no-price-tag">
                                                    중개수수료 <?=$commission_fee?>
                                                </div>
                                        ...
                                        <?php
                                        // 계약 상태 클래스
                                        $contractClass = $v['is_contract'] == 1
                                            ? 'no-state-contract-done'
                                            : 'no-state-contract-available';

                                        // 노출 상태 클래스
                                        $visibleClass = $v['is_visible'] == 1
                                            ? 'no-state-visible-true'
                                            : 'no-state-visible-false';
                                        ?>
                                        <div class="no-gallery-item-txt">
                                            <div class="no-state-btn-wrap">

                                                <div class="no-state-btn no-state-visible-btn" data-id="<?= $v['no'] ?>"
                                                    data-type="is_visible">
                                                    <button type="button" class="<?= $visibleClass ?>">
                                                        <?= $is_visible[$v['is_visible']] ?>
                                                        <i class="fa-solid fa-caret-down"></i>
                                                    </button>
                                                    <div class="no-state-options">
                                                        <?php foreach ($is_visible as $key => $label): ?>
                                                        <button type="button"
                                                            data-value="<?= $key ?>"><?= $label ?></button>
                                                        <?php endforeach; ?>
                                                    </div>
                                                </div>

                                                <div class="no-state-btn no-state-contract-btn"
                                                    data-id="<?= $v['no'] ?>" data-type="is_contract">
                                                    <button type="button" class="<?= $contractClass ?>">
                                                        <?= $is_contract[$v['is_contract']] ?>
                                                        <i class="fa-solid fa-caret-down"></i>
                                                    </button>
                                                    <div class="no-state-options">
                                                        <?php foreach ($is_contract as $key => $label): ?>
                                                        <button type="button"
                                                            data-value="<?= $key ?>"><?= $label ?></button>
                                                        <?php endforeach; ?>
                                                    </div>
                                            ...
                                            <div class="no-gallery-item-txt-wrap">
                                                <div>
                                                    <h2><?=$v['title']?></h2>
                                                    <p>(월세 <?=$pay_for_month?>)</p>
                                                </div>
                                                <span><?=$address?></span>
                                            </div>
                                            <div class="no-gallery-item-systems">
                                                <ul>
                                                    <li>
